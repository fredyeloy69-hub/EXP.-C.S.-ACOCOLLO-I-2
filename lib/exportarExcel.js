...
// más profundo el nivel: nivel 1 el más oscuro/saturado, nivel 3 el más
// claro — para que la jerarquía se note tanto en el color como en el tono.
const COLORES_NIVEL_SEPARADOR = [
  { base: DORADO_FASE2, factor: 0.35 }, // nivel 1: dorado, oscuro (predominante)
  { base: "FF2E5FA3", factor: 0.55 },   // nivel 2: azul, más claro
];
 ...
 * archivadores (en blanco, para completar a mano) y un correlativo de orden.
 *
 * @param {string} areaNombre - nombre del área (carpeta madre) a exportar
 * @param {Array} carpetasDelArea - carpetas (ya filtradas a esa área)
 */
export async function generarListaGeneralExcel(areaNombre, carpetasDelArea) {
  ...
  let contadorOrden = 1;
  for (const item of items) {
    const fila = sheet.addRow([contadorOrden++, item.numero, item.nombre, ""]);
  ...
  sheet.views = [{ state: "frozen", ySplit: FILA_ENCABEZADO }];
  sheet.headerFooter.oddFooter = "&L&I[Sistema ACOCOLLO I-2] Índice general de la documentación&R&IPágina &P de &N";
