...
export async function generarReporteExcelPorArea(areaNombre, carpetasDelArea, tipoFiltro = "todas") {
  ...
  function agregarSubEncabezado(nombre, nivel, items) {
    ...
    // plana, al mismo nivel, quedan exactamente del mismo color.
    const factor = FACTORES_HOJA[Math.min(nivel - 1, FACTORES_HOJA.length - 1)];
    celda.font = { bold: true, size: esNivel1 ? 13 : 11, color: { argb: esNivel1 ? "FFFFFFFF" : "FF1A3840" } };
    celda.fill = {
      type: "pattern",
      pattern: "solid",
      fgColor: { argb: esNivel1 ? VERDE_AZULADO : mezclarConBlanco(VERDE_AZULADO, factor) },
    };
...
// Degradado de UN solo color (el dorado del proyecto) que se aclara mientras
// más profundo el nivel — mismo criterio que usa Visor Chijnaya: el nivel 1
// es el más nítido (fondo sólido), y cada nivel más adentro se ve más tenue.
const FACTORES_NIVEL_SEPARADOR = [0, 0.55, 0.72, 0.85, 0.92];
function colorNivelSeparador(nivel) {
  const factor = FACTORES_NIVEL_SEPARADOR[Math.min(nivel - 1, FACTORES_NIVEL_SEPARADOR.length - 1)];
...
export async function generarListaSeparadoresExcel(areaNombre, carpetasDelArea) {
  ...
  for (const item of items) {
    ...
    if (item.nivel === 1) {
      ...
      // no solo negrita sobre blanco, igual criterio que Chijnaya.
      fila.eachCell({ includeEmpty: true }, (cell) => {
        cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: DORADO_FASE2 } };
        cell.font = { bold: true, color: { argb: "FFFFFFFF" } };
      });
    } else {
      // A partir del nivel 2, el sombreado tenue depende SOLO del nivel —
      // todos los ítems del mismo nivel se ven igual, tengan o no subcarpetas
      // propias (antes solo se sombreaban los que tenían hijos, dejando sin
      // sombra a ítems como "2.7 COTIZACIONES" que están al mismo nivel que
      // "2.6 RELACIÓN DE INSUMOS" pero no tienen nada adentro).
